...
`GrowGraph` используется как companion layer для структуры, readiness,
semantic preservation, effectiveness и federation contracts. Он не заменяет
`SKILL.md`, `rules/`, `references/` и scripts.

Перед claim `GGA9` запусти:

```bash
python3 scripts/growgraph_contract_gate.py
```

Generated artifacts do not grant canonical writes.
